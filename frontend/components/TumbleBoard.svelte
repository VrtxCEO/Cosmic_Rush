...
<script lang="ts">
	...
	context.eventEmitter.subscribeOnMount({
		...
		tumbleBoardExplode: async ({ explodingPositions }) => {
			// Deduplicate positions — the same cell can appear in multiple winning clusters.
			// Without this, the second waitForResolve overwrites oncomplete on the shared
			// symbol object, leaving the first promise pending forever (game freeze).
						...
						return tumbleReel.map(async (tumbleSymbol, symbolIndex) => {
							...
							if (targetY !== tumbleSymbol.symbolY.current) {
								...
								if (symbolIndex > 0 && symbolIndex < tumbleReel.length - 1) {
									...
									context.stateGameDerived.onSymbolLand({ rawSymbol: tumbleSymbol.rawSymbol, symbolIndexOfBoard: symbolIndex - 1 });
